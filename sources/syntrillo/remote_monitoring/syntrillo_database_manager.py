    ...
    def insert_overwrite_healthie_form_templates(self, data_json: dict) -> dict:
        ...
        try:
            with self.conn.cursor() as cursor:
                ...
                cursor.execute(
                    """
                        REPLACE INTO healthie_form_templates (
                            form_id,
                            form_name,
                            module_id,
                            module_label,
                        ...
                        SELECT
                            ...
                            NULLIF(TRIM(cf.module_label), '') as module_label,
                            NULLIF(TRIM(cf.module_options), '') as module_options
                        FROM JSON_TABLE(
                            JSON_EXTRACT(%s, '$.customModuleForms'),
                            '$[*]' COLUMNS (
                                id VARCHAR(255) PATH '$.id',
                                name VARCHAR(255) PATH '$.name',
                                NESTED PATH '$.custom_modules[*]' COLUMNS (
                                    module_id VARCHAR(255) PATH '$.id',
                                    mod_type VARCHAR(255) PATH '$.mod_type',
                                    module_label TEXT PATH '$.label',
                                    module_options TEXT PATH '$.options'
                                )
                            )
                        ) as cf
                        ORDER BY form_id, module_id;
                    """,
                    (data_json)
                )
                self.conn.commit()
    ...
    def insert_overwrite_healthie_form_responses(self, data_json: dict) -> dict:
            ...
            try:
                with self.conn.cursor() as cursor:
                    ...
                    cursor.execute(
                        """
                            REPLACE INTO healthie_form_responses (
                                module_id,
                                form_id,
                                user_id,
                                answer,
                            ...
                            SELECT
                                fa.custom_module_id as module_id,
                                fa.form_id,
                                fa.user_id,
                                NULLIF(TRIM(fa.displayed_answer), '') as answer,
                                fa.created_at
                            FROM JSON_TABLE(
                                %s,
                                '$.formAnswerGroups[*]' COLUMNS (
                                    form_id VARCHAR(255) PATH '$.custom_module_form.id',
                                    NESTED PATH '$.form_answers[*]' COLUMNS (
                                        custom_module_id VARCHAR(255) PATH '$.custom_module.id',
                                        user_id VARCHAR(255) PATH '$.user_id',
                                        displayed_answer TEXT PATH '$.displayed_answer',
