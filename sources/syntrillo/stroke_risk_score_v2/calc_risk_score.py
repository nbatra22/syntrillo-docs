...
from syntrillo.stroke_risk_score_v2.models.srs_form import AnemiaSeverityOptions, ArterialClotOccurrencesOptions, CADTypeOptions, EjectionFractionOptions, HDLLevelOptions, LDLLevelOptions, LikelihoodOfTIAOptions, NumberOfStrokesOptions, OSASeverityOptions, PFOPresenceOptions, SRSFormResponse, StenosisPercentageOptions, StrokeMechanismOptions, TriglyceridesLevelOptions, VenousClotOccurrencesOptions

# Dependent Risk Factors
weighting = {
    "value": {
        LOW_VALUE: 1.25,
        LOW_INTERMEDIATE_VALUE: 1.5,
    ...
    "treatment_optimization": {
        TREATMENT_OPTIMIZATION: 1,
        TREATMENT_PARTIALLY_OPTIMIZED: 0.5,
        TREATMENT_NOT_OPTIMIZED: 0.5,
    }
}
...
def calculate_risk_score(syntrillo_internal_key: uuid.UUID):
    ...
    srs_response_form = agg_data.get("srs_response_data", None)

    dependent_risk_factor_values, dependent_efficacy_values, dependent_optimization_values = calculate_dependent_risk_factors(srs_response_form)
...
def calculate_dependent_risk_factors(most_recent_srs_form_response: SRSFormResponse) -> dict:
    """
    ...
    dependent_optimization_values = 0. # TODO: Currently not supported due to lack of data.
    ...
    return dependent_risk_factor_values, dependent_efficacy_values, dependent_optimization_values
...
def get_independent_risk_factor_values(agg_data: dict):
    ...
    # Data from Healthie
    avg_trailing_rhr_value = healthie_srs_data.get("average_rhr_trailing", None)
    avg_trailing_inactivity_value = healthie_srs_data.get("inactivity_minutes_answer", None)
    # Baseline data NOT USED IN SRS CALCULATION – average_rhr_baseline_value = healthie_srs_data["average_rhr_baseline"]
...
if __name__ == "__main__":
    ...
    calculate_risk_score(syntrillo_internal_key)