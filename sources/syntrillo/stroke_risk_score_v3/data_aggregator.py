...
from syntrillo.stroke_risk_score_v3.utils import get_biometric_data, fetch_all_form_responses_from_healthie
from syntrillo.api_healthie.utils import HealthieUtils
from syntrillo.stroke_risk_score_v3.models.srs_form import (
    SRSFormResponse,
    GenderOptions,
...
from syntrillo.stroke_risk_score_v3.models.lab_data import LabData
...
from syntrillo.stroke_risk_score_v3.constants import (
    CREATED_AT,
    TYPE_BP,
    TYPE_RHR,
    TYPE_HR,
...
class SRSDataAggregator:
    """
    Class responsible for aggregating data for Stroke Risk Score calculations.

    Attributes:
        syntrillo_internal_key (uuid.UUID): Unique identifier for the patient.
        healthie_user_id (str): Healthie user ID for the patient.
        db_manager (SyntrilloDatabaseManager): Database manager instance.
        healthie_utils (HealthieUtils): Healthie utilities instance.
    ...
    def __init__(self, syntrillo_internal_key: uuid.UUID):
        ...
        self.db_manager = SyntrilloDatabaseManager(syntrillo_internal_key)
        ...
        self.healthie_user_id = entry.get('healthie_user_id') if entry else None

    def aggregate_data(self, is_ondemand_srs: bool = False, gender: GenderOptions = GenderOptions.MAN) -> dict:
        """
        Aggregate data from Tenovi, Healthie, and SRS response data

        Args:
            is_ondemand_srs (bool): Whether this is an on-demand SRS calculation
            gender (GenderOptions): Patient gender for on-demand SRS
        ...
        """
        try:
            logger.info(f"Beginning to aggregate data for patient with syntrillo_internal_key {self.syntrillo_internal_key} ...")

            if not self.healthie_user_id:
                return {}

            tenovi_bp_data = self.get_tenovi_bp_data()
            tenovi_hr_data = self.get_tenovi_hr_data()
            healthie_srs_data = self.get_srs_healthie_data()
            lab_data = self.get_lab_data()

            if is_ondemand_srs:
                patient_history_data = self.get_patient_history_data()
                srs_response_data = [
                    SRSFormResponse(
                        syntrillo_internal_key_patient=str(self.syntrillo_internal_key),
                        created_at=datetime.now(),
                        Gender=gender,
                    )
                ]
                srs_response_data[0] = self.populate_with_patient_history(srs_response_data[0], patient_history_data)
            else:
                srs_response_data = self.get_srs_response_data()
    ...
    def populate_with_patient_history(self, srs_response_data: SRSFormResponse, patient_history_data: dict) -> SRSFormResponse:
        """Populate SRS response data with patient history from Healthie forms."""
        ...
        biometric_data = get_biometric_data(self.syntrillo_internal_key)
    ...
    def get_patient_history_data(self) -> dict:
        """
        Fetch the patient's responses to the Healthie form "Onboarding Record Review w/ Patient [v9.2]"

        Returns:
            dict: Patient history data extracted from form responses
        """
        ...
        for answer_group in all_form_groups:
            ...
            if answer_group.get('form_answers'):
                # Check the user_id of the first answer – assuming all answers in a group have same user_id
                first_answer = answer_group['form_answers'][0]
                if first_answer.get('user_id') == self.healthie_user_id:
    ...
    def get_srs_healthie_data(self) -> dict:
        """
        ...
        try:
            ...
            rhr_data = self.get_healthie_metric_data(category=RHR_CATEGORY)
            rhr_metadata = self.calc_rhr_metadata(rhr_data) if rhr_data else {}

            # Retreive the activity data
            hours_sitting_data = self.get_healthie_metric_data(category=HOURS_SITTING_CATEGORY)
            recent_hours_sitting_entry = hours_sitting_data[-1] if hours_sitting_data else {}

            physical_activity_minutes_data = self.get_healthie_metric_data(category=PHYSICAL_ACTIVITY_MINS_CATEGORY)
            recent_physical_activity_minutes_entry = physical_activity_minutes_data[-1] if physical_activity_minutes_data else {}

            recent_ssq_data = self.get_healthie_metric_data(category=SSQ_CATEGORY)
    ...
    def get_healthie_activity_and_inactivity_module_ids(self) -> dict:
        """
        ...
        Returns:
            dict: Module IDs for activity and inactivity forms
        """
        _, module_id_inactivity_intake = self.db_manager.get_form_module_ids_by_module_label(INACTIVITY_INTAKE_MODULE_LABEL)
        _, module_id_inactivity_charting = self.db_manager.get_form_module_ids_by_module_label(INACTIVITY_CHARTING_MODULE_LABEL)

        _, module_id_activity_intake = self.db_manager.get_form_module_ids_by_module_label(ACTIVITY_INTAKE_MODULE_LABEL)
        _, module_id_activity_charting = self.db_manager.get_form_module_ids_by_module_label(ACTIVITY_CHARTING_MODULE_LABEL)
    ...
    def get_healthie_activity_data(self) -> dict:
        """
        Get the activity data from healthie

        Returns:
            dict: inactivity_minutes_answer and activity_minutes_answer
        Raises:
        ...
        try:
            ...
            physical_activity_module_ids = self.get_healthie_activity_and_inactivity_module_ids()
            ...
            syntrillo_internal_key_str = str(self.syntrillo_internal_key)

            # Retreive the intake & charting inactivity value based on the module id and healthie user id
            intake_inactivity_hours_answer, intake_updated_at = self.db_manager.get_patient_form_response_by_module_id(module_id_inactivity_intake, syntrillo_internal_key_str)
            charting_inactivity_hours_answer, charting_updated_at = self.db_manager.get_patient_form_response_by_module_id(module_id_inactivity_charting, syntrillo_internal_key_str)

            intake_activity_minutes_answer, intake_activity_updated_at = self.db_manager.get_patient_form_response_by_module_id(module_id_activity_intake, syntrillo_internal_key_str)
            charting_activity_minutes_answer, charting_activity_updated_at = self.db_manager.get_patient_form_response_by_module_id(module_id_activity_charting, syntrillo_internal_key_str)
    ...
    def calc_rhr_metadata(
        ...
        try:
            ...
            baseline_df = self.get_timeframed_data(rhr_df, baseline_start, baseline_end, TYPE_RHR)
            ...
            if self.is_valid_trailing_timeframe_dates(trailing_start, baseline_end):
                trailing_df = self.get_timeframed_data(rhr_df, trailing_start, trailing_end, TYPE_RHR)
            ...
            if self.is_valid_trailing_timeframe_dates(prior_start, baseline_end):
                prior_df = self.get_timeframed_data(rhr_df, prior_start, prior_end, TYPE_RHR)
    ...
    def calc_pulse_metadata(
        ...
        try:
            ...
            # Ensure the baseline dataframe is valid
            baseline_df = self.get_timeframed_data(pulse_df, baseline_start, baseline_end, TYPE_PULSE)
            baseline_average = baseline_df[METRIC_STAT].mean() if not baseline_df.empty else None
            ...
            trailing_average = None
            if self.is_valid_trailing_timeframe_dates(trailing_start, baseline_end):
                trailing_df = self.get_timeframed_data(pulse_df, trailing_start, trailing_end, TYPE_PULSE)
                trailing_average = trailing_df[METRIC_STAT].mean() if not trailing_df.empty else None # Calculate the average trailing for the rhr_data
            else:
                logger.warning("Not enough data to calculate trailing average...")
            ...
            # Check if the baseline timeframe overlaps with the prior timeframe
            if self.is_valid_trailing_timeframe_dates(prior_start, baseline_end):
                prior_df = self.get_timeframed_data(pulse_df, prior_start, prior_end, TYPE_PULSE)
                prior_average = prior_df[METRIC_STAT].mean() if not prior_df.empty else None # Calculate the average prior for the rhr_data
            else:
                logger.warning("Not enough data to calculate prior average...")
    ...
    def get_srs_response_data(self) -> Union[SRSFormResponse, None]:
        """
        ...
        """
        try:
            srs_form_responses = self.db_manager.get_srs_form_responses(self.syntrillo_internal_key)
    ...
    def get_tenovi_hr_data(self) -> dict:
        """
        ...
        try:
            ...
            hr_measurements, _ = self.db_manager.get_latest_measurements(metric_name=PULSE_METRIC_NAME)
            ...
            trailing_df = self.get_timeframed_data(hr_df, trailing_start, trailing_end, TYPE_HR)
    ...
    def get_tenovi_bp_data(self) -> dict:
        """
        ...
        try:
            ...
            baseline_bp_df = self.get_baseline_bp_data(bp_df, baseline_start=baseline_start, baseline_weeks=BASELINE_NUM_WEEKS) # Get the baseline data
            trailing_bp_df = self.get_trailing_bp_data(bp_df, baseline_start=baseline_start, trailing_weeks=TRAILING_NUM_WEEKS, trailing_days=TRAILING_NUM_DAYS) # Get the trailing data

            # Calculate the bp metadata for the trailing and baseline dataframes
            bp_metadata = self.calc_bp_metadata(bp_analysis, trailing_bp_df, baseline_bp_df)
    ...
    def get_baseline_bp_data(self, bp_dataframe: pd.DataFrame, baseline_start: datetime, baseline_weeks: int = 2) -> dict:
        """
        ...
        try:
            ...
            baseline_df = self.get_timeframed_data(bp_dataframe, baseline_start, baseline_end, TYPE_BP)
    ...
    def get_trailing_bp_data(self, bp_dataframe: pd.DataFrame, baseline_start: datetime, trailing_weeks: int = 4, trailing_days: int = 0) -> dict:
        """
        ...
        try:
            ...
            if not self.is_valid_trailing_timeframe_dates(trailing_start, baseline_start):
                logger.error("Trailing data is not valid")
                # raise ValueError("Trailing data is not valid")
                return None

            # Get the trailing dataframe
            trailing_df = self.get_timeframed_data(bp_dataframe, trailing_start, trailing_end, TYPE_BP)
    ...
    def calc_bp_metadata(self, bp_analysis: BloodPressureAnalysis, trailing_bp_dataframe: pd.DataFrame, baseline_bp_dataframe: pd.DataFrame) -> dict:
        """
    ...
    def get_timeframed_data(self, dataframe: pd.DataFrame, timeframe_start: datetime, timeframe_end: datetime, measurement_type: str) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if not self.is_valid_timeframe_num_measurements(timeframe_df):
    ...
    def get_lab_data(self) -> LabData:
        ...
        try:
            ...
            ldl_data = self.get_healthie_metric_data(category=LDL_CATEGORY)
            hdl_data = self.get_healthie_metric_data(category=HDL_CATEGORY)
            creatintine_data = self.get_healthie_metric_data(category=CREATINTINE_CATEGORY)
            hgA1c_data = self.get_healthie_metric_data(category=HGA1C_CATEGORY)
            hsCRP_data = self.get_healthie_metric_data(category=HSCRP_CATEGORY)
            hemoglobin_data = self.get_healthie_metric_data(category=HEMOGLOBIN_CATEGORY)
