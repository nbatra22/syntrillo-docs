...
from syntrillo.stroke_risk_score_v3.models.treatment_compliance import TreatmentComplianceOptions

class SRSCalculator:
    """
    Responsible for calculating a patient's risk score and priority score.
    """
    ...
    def __init__(self, syntrillo_internal_key: uuid.UUID):
        ...
        self.db_manager = SyntrilloDatabaseManager(syntrillo_internal_key=self.syntrillo_internal_key)
    ...
    def calculate_risk_score(syntrillo_internal_key: uuid.UUID, is_ondemand_srs: bool = False) -> tuple[float, dict, float]:
        ...
        """
        try:
            # Use SRSDataAggregator to aggregate data
            data_aggregator = SRSDataAggregator(syntrillo_internal_key)
            agg_data = data_aggregator.aggregate_data(is_ondemand_srs=is_ondemand_srs)
            ...
            calculator = SRSCalculator(syntrillo_internal_key)

            independent_risk_factor_value, stroke_priority_score_total, independent_risk_variable_scores = calculator.calculate_independent_srs_values(agg_data)

            dependent_risk_values = calculator.calculate_dependent_risk_factors(agg_data)
            ...
            independent_risk_variable_scores, dependent_risk_variable_contributions = calculator.calc_risk_variable_contributions(
    ...
    def calculate_dependent_risk_factors(self, agg_data: dict) -> dict:
        """
        ...
        try:
            ...
            if most_recent_srs_form_response.HasPreviousStroke:
                if most_recent_srs_form_response.NumberOfStrokes == NumberOfStrokesOptions.ONE:
                    if most_recent_srs_form_response.LatestStrokeMechanism == StrokeMechanismOptions.SMALL_VESSEL:
                        dependent_risk_factor_value = self.weighting[VALUE][HIGH_VALUE]
                        dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][HIGH_EFFICACY]
                    ...
                    if most_recent_srs_form_response.LatestStrokeMechanism == StrokeMechanismOptions.SMALL_VESSEL:
                        ...
                        dependent_risk_factor_value = self.weighting[VALUE][HIGH_VALUE]
                        dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                    else:
                        dependent_risk_factor_value = self.weighting[VALUE][HIGH_VALUE]
                        dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]

                if compliance_data and compliance_data.strokeCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.strokeCompliance]
                else: # Default to Moderate
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.ScreenedForTIA and most_recent_srs_form_response.LikelihoodOfTIA == LikelihoodOfTIAOptions.HIGH_LIKELIHOOD:
                ...
                if compliance_data and compliance_data.tiaCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.tiaCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HasPriorHeadCT and most_recent_srs_form_response.ChronicInfarctPresent:
                ...
                if compliance_data and compliance_data.chronicInfarctCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.chronicInfarctCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfAtrialFibrillation:
                ...
                if compliance_data and compliance_data.atrialFibrillationCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.atrialFibrillationCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfIronDeficiencyAnemia:
                if most_recent_srs_form_response.AnemiaSeverity == AnemiaSeverityOptions.MILD:
                    dependent_risk_factor_value = self.weighting[VALUE][LOW_VALUE]
                    dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                elif most_recent_srs_form_response.AnemiaSeverity == AnemiaSeverityOptions.SEVERE:
                    dependent_risk_factor_value = self.weighting[VALUE][LOW_INTERMEDIATE_VALUE]
                    dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                else:
                    dependent_risk_factor_value = self.weighting[VALUE][LOW_INTERMEDIATE_VALUE]
                    dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                ...
                if compliance_data and compliance_data.ironDeficiencyAnemiaCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.ironDeficiencyAnemiaCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfArterialClots:
                if most_recent_srs_form_response.ArterialClotOccurrences == ArterialClotOccurrencesOptions.SINGLE_PRIOR_EVENT:
                    ...
                    dependent_risk_factor_value = self.weighting[VALUE][INTERMEDIATE_HIGH_VALUE]
                    dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                else:
                    dependent_risk_factor_value = self.weighting[VALUE][LOW_INTERMEDIATE_VALUE]
                    dependent_efficacy_value = self.weighting[TREATMENT_EFFICACY][MODERATE_EFFICACY]
                ...
                if compliance_data and compliance_data.arterialClotsCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.arterialClotsCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfVenousClots:
                ...
                if compliance_data and compliance_data.venousClotsCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.venousClotsCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfCHF:
                ...
                if compliance_data and compliance_data.chfCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.chfCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfCarotidStenosis:
                ...
                if compliance_data and compliance_data.carotidStenosisCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.carotidStenosisCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfOSA:
                ...
                if compliance_data and compliance_data.osaCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.osaCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfValvularHeartDisease:
                ...
                if compliance_data and compliance_data.valvularHeartDiseaseCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.valvularHeartDiseaseCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HistoryOfCAD:
                ...
                if compliance_data and compliance_data.cadCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.cadCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
                dependent_section_score = ((dependent_risk_factor_value-1)*(1-(dependent_efficacy_value*dependent_optimization_value)))+1
            ...
            if most_recent_srs_form_response.LDLLevel:
                ...
                if compliance_data and compliance_data.ldlCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.ldlCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
            ...
            if most_recent_srs_form_response.HDLLevel:
                ...
                if compliance_data and compliance_data.hdlCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.hdlCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
                dependent_section_score = ((dependent_risk_factor_value-1)*(1-(dependent_efficacy_value*dependent_optimization_value)))+1
            ...
            if most_recent_srs_form_response.TriglyceridesLevel:
                ...
                if compliance_data and compliance_data.triglyceridesCompliance:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][compliance_data.triglyceridesCompliance]
                else:
                    dependent_optimization_value = self.weighting[TREATMENT_OPTIM][TreatmentComplianceOptions.PARTIALLY_OPTIMIZED]
    ...
    def calculate_independent_srs_values(self, agg_data: dict) -> tuple[float, float]:
        """
        Calculates the risk score values associated with objective metrics.
        """
        independent_risk_factor_values = self.get_independent_risk_factor_values(agg_data)
        independent_risk_score_total, stroke_priority_score_total, independent_risk_variable_scores = self.get_independent_risk_score_value(independent_risk_factor_values)
        return independent_risk_score_total, stroke_priority_score_total, independent_risk_variable_scores


    # Independent Risk Factors
    def get_independent_risk_factor_values(self, agg_data: dict):
        """
        Calculates the risk score values associated with objective metrics.
    ...
    def get_independent_risk_score_value(self, independent_risk_factors: dict) -> tuple[float, float]:
        """
            ...
            for category, value in independent_risk_factors.items():
                ...
                if category in self.EXCLUDED_CATEGORIES:
                    ...
                    if type(value) is np.float64:
                        value = float(value)
                    risk_values = self.db_manager.get_srs_value_by_category_and_value(category, value, gender=independent_risk_factors.get(GENDER, None))
                else:
                    # Get most likely risk value for the factor
                    risk_values = self.db_manager.get_srs_value_by_category_and_value(category, gender=independent_risk_factors.get(GENDER, None))
